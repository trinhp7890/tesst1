  ...
  Cart.prototype = _.assignIn({}, Cart.prototype, {
    ...
    _setQuantityInputs: function($item, item, index) {
      ...
      $(selectors.quantityLabelDesktop, $item).attr(
        ...
        var cartitemcountall = cart.item_count;
        
        console.log(cartitemcountall + ' :22: ' + index);       
        var cartItemCounter = document.getElementById('count_item_cart');
        cartItemCounter.innerHTML = cartitemcountall;
        if (cartitemcountall > 0) {                  
          $('#count_item_cart').show();
        }
        else {
          $('#count_item_cart').hide();
        }
      });
    },
    ...
    _onRemoveItem: function(evt) {
      ...
      jQuery.getJSON('/cart.js', function (cart) {
        var cartitemcountall = cart.item_count - cart.items[index -1].quantity;
        var cartItemCounter = document.getElementById('count_item_cart');
        cartItemCounter.innerHTML = cartitemcountall;
        if (cartitemcountall > 0) {                  
          $('#count_item_cart').show();
...
$(document).ready(function() {
  ...
  thumbnailss = $('img[src*="/products/"]');
  thumbnails_ = $('img[class*="product-single__thumbnail-image img_products_media des"]');
  thumbnails = $('img[class*="product-single__thumbnail-image img_products_media mb"]');
  
  if (thumbnailss.length) {
    thumbnailss.bind('click', function() {
      var arrImage = $(this).attr('src').split('?')[0].split('.');
      var strExtention = arrImage.pop();
      var strRemaining = arrImage.pop().replace(/_[a-zA-Z0-9@]+$/,'');
      var strNewImage = arrImage.join('.')+"."+strRemaining+"."+strExtention;
      if (typeof variantImages !== 'undefined' && typeof variantImages !== 'strNewImage' &&   typeof variantImages[strNewImage] !== 'undefined') {
  ...
  if (thumbnails_.length) {
    thumbnails_.bind('click', function () {
      thumbnails_.removeClass('border_Image');
      $(this).addClass('border_Image');
      if ($(this)[0].name == 0) {
          $(".badge-icon").css("display","block");
      }else{
          $(".badge-icon").css("display","none");
      }
    });
  }
  if (thumbnails.length) {
    thumbnails.bind('click', function() {
      	    if ($(this)[0].name == 0) {
          $(".badge-icon-mb").css("display","block");
      }else{
          $(".badge-icon-mb").css("display","none");
      }      
    });
  }
});